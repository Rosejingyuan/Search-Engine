...
def get_query_url(query):
    ...
    if len(query_list) == 1:
        ...
        index_file = "/users/jingyuan_wang/PycharmProjects/SearchEngine-master/result/" + query_term + ".json"
        index = get_json(index_file)
        ...
        for term in query_list:
            try:
                index_file = "/users/jingyuan_wang/PycharmProjects/SearchEngine-master/result/" + term + ".json"
                index = get_json(index_file)
                entry = index[term]
                documents = entry.keys()
                for d in documents:
            ...
            MapFile = "/Users/jingyuan_wang/Downloads/WEBPAGES_RAW/bookkeeping.json"
            hash = get_json(MapFile)
            with open('/users/jingyuan_wang/PycharmProjects/SearchEngine-master/query_toplinks/' + query_list[0] + '_' + query_list[1] + '.txt', 'w') as f:
                for pair in ranked_score:
                    baseUrl = hash[pair[0].decode('utf-8')].encode('utf-8')
                    absUrl = 'https://' + baseUrl
                    url_list.append(absUrl)
...
def interface():
    ...
    var = StringVar()
    query_ety = Entry(root, textvariable = var, font="Times 15")
